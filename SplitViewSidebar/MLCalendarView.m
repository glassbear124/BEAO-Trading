...
//        _arrSelReminders = [[NSMutableArray alloc] init];
        
        [self commonInit];
        
        MLCalendarBackground* bv = (MLCalendarBackground*)self.view;
        bv.isMultiSelect = true;        
...
- (void) commonInit {
    ...
    if( self.isMultiSelect == false )
        ...
        self.selectionColor = [Utility rColor];
...
- (void)cellClicked:(id)sender {
    ...
    if( self.isMultiSelect == false ) {
        ...
        if(self.delegate) {
            if([self.delegate respondsToSelector:@selector(didReminderSelectDate:)]) {
                [self.delegate didReminderSelectDate:self.selectedDate];
            }
        }
    }
}
		...
		for(; col < 7; col++) {
			if(day <= lastDay) {
                ...
                if( self.isMultiSelect == false ) {
                    ...
                    NSString* cellStr = [Utility dateToStr:cell.representedDate];
                    for( Reminder* reminder in _arrReminders ) {
                        if( [cellStr isEqualToString:reminder.date] == true ) {
                            cell.isReminder = true;
                            break;
                        }
                    }
                }
