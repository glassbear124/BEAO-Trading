	...
	if(self.owner) {
		...
		if(self.representedDate) {
            ...
            if( self.isReminder ) {
                ...
                NSBezierPath* bottomLine = [NSBezierPath bezierPath];
                CGFloat pattern[] = {2, 2};
                [bottomLine setLineDash:pattern count:2 phase:2];
                [bottomLine moveToPoint:NSMakePoint(NSMinX(bounds)+3, NSMaxY(bounds))];
                [bottomLine lineToPoint:NSMakePoint(NSMaxX(bounds)-3, NSMaxY(bounds))];
                bottomLine.lineWidth = 6.0f;
                [bottomLine stroke];
            }
		}
